...
Replay can send recorded payloads to a server. Graybox automatically reuses a saved target only for `localhost`, `*.localhost` names, and loopback IP addresses. Remote replay requires an explicit `--target` or `--unsafe-original-target`. Review the destination and recording before allowing it. Redirects are reported without being followed, and truncated or incomplete request bodies are refused rather than sent partially.
