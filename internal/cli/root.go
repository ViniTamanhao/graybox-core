...
func (a App) Run(ctx context.Context, args []string) int {
	...
	switch args[0] {
	case "record":
		code, err = a.runRecord(ctx, args[1:])
	case "ls":
		code, err = a.runList(ctx, args[1:])
	case "show":
		code, err = a.runShow(ctx, args[1:])
	case "replay":
		code, err = a.runReplay(ctx, args[1:])
	case "version":
		code, err = a.runVersion(args[1:])
	case "help", "-h", "--help":
		if len(args) > 1 {
		...
		return ExitSuccess
	default:
		fmt.Fprintf(a.Stderr, "graybox: unknown command %q\n\n", args[0])
...
func (a App) printRootHelp(w io.Writer) {
	fmt.Fprint(w, `Graybox — a local-first API flight recorder
...
func (a App) helpCommand(command string) int {
	switch command {
	case "record":
		a.runRecord(context.Background(), []string{"--help"})
	case "ls":
		a.runList(context.Background(), []string{"--help"})
	case "show":
		a.runShow(context.Background(), []string{"--help"})
	case "replay":
		a.runReplay(context.Background(), []string{"--help"})
	case "version":
		a.runVersion([]string{"--help"})
	default:
		fmt.Fprintf(a.Stderr, "graybox: unknown command %q\n", command)
		return ExitUsage
	}
...
func parseInterspersed(fs *flag.FlagSet, args []string, boolean map[string]bool) ([]string, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		if strings.Contains(arg, "=") || boolean[name] || name == "h" || name == "help" {
			continue
		}
		if i+1 >= len(args) {
...
func classifyError(err error) int {
	...
	if errors.Is(err, storage.ErrInvalidRecording) || errors.Is(err, storage.ErrUnsupportedSchema) {
...
func parseID(value string) (int64, error) {
	...
	if err != nil || id <= 0 {
		return 0, usageError{fmt.Sprintf("exchange ID %q must be a positive integer", value)}
	}
	return id, nil
}
